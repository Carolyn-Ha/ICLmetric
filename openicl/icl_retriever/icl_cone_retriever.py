...
class ConERetriever(TopkRetriever):
    ...
    def __init__(self,
        ...
        self.model_tokenizer = AutoTokenizer.from_pretrained(model_tokenizer_name, token=os.getenv('HF_TOKEN'))
    ...
    def cal_ce(self, input_texts: List[str], mask_lengths=None, test_lengths=None):
        ...
        if self.metric_model is None:
            ...
            # NOTE: quantization applied
            bnb_config = BitsAndBytesConfig(**quantization)
            if 'Llama' in self.ce_model_name:
                self.metric_model = LlamaForCausalLM.from_pretrained(self.ce_model_name, quantization_config=bnb_config)
            else:
                self.metric_model = AutoModelForCausalLM.from_pretrained(self.ce_model_name, quantization_config=bnb_config)
