...
from transformers import AutoTokenizer, AutoModelForCausalLM, PretrainedConfig, GPT2Tokenizer, AutoConfig, \
    T5ForConditionalGeneration, BitsAndBytesConfig
...
# NOTE
TOKEN = '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'

# TODO how to control quantization config?
quantization = {
    'load_in_4bit': True,
    'load_in_8bit': False,
...
class BaseInferencer:
    ...
    def __get_hf_model_from_name(self, model_name):
        if 't5' in model_name:
            ...
            # NOTE : load with efficiently
            bnb_config = BitsAndBytesConfig(**quantization)
            return LlamaForCausalLM.from_pretrained(model_name, quantization_config=bnb_config, token=TOKEN)
        else:
            bnb_config = BitsAndBytesConfig(**quantization)
            return AutoModelForCausalLM.from_pretrained(model_name, quantization_config=bnb_config, token=TOKEN)

    def __get_hf_model_from_config(self, model_name, model_config):
        if 't5' in model_name:
            raise TypeError("T5 model has no 'from_config' method")
        elif 'Llama' in model_name:
            return LlamaForCausalLM.from_pretrained(model_config, token=TOKEN)
        else:
            return AutoModelForCausalLM.from_config(model_config, token=TOKEN)
    ...
    def __init_tokenizer(self, tokenizer_name):
        if self.api_name == 'opt-175b':
            ...
            #     self.tokenizer = LlamaTokenizer.from_pretrained(tokenizer_name, token=TOKEN)
            # else:
