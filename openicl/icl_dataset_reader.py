...
class DatasetReader:
    ...
    Attributes:
        ...
        references(:obj:`list`, optional): The list of references, initialized by ``self.dataset[self.test_split][self.output_column]``.
            # reference: test column에 해당하는 data의 출력 컬럼을 저장해두고, 나중에 모델을 평가할 때 참조용으로 사용
    ...
    def set_references(self, column: str, split: Optional[str] = None) -> None: # test column의 출력값을 저장하는 부분
    ...
    def generate_input_field_prompt(self, entry: Dict) -> str: # 단일 데이터 항목에 대한 프롬프트 생성
        ...
        prompt = None
        logger.debug(entry)
        logger.debug(self.input_columns)
    ...
    def generate_input_field_corpus(self, dataset: Union[Dataset, DatasetDict], split: Optional[str] = None) -> List[
        ...
        for entry in dataset:
            corpus.append(self.generate_input_field_prompt(entry)) # 위에서 생성한 function을 활용해 전체 데이터셋에 대한 프롬프트 생성 & 리스트로 반환
        return corpus
